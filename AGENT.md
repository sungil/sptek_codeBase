...
## AI 에이전트 문서 사용 원칙
`AGENTS.md`, `AGENT.md`, `CLAUDE.md`는 모두 AI 에이전트를 위한 작업 지침 문서로 같은 성격의 문서로 취급한다. Codex, Claude 등 어떤 Agentic Coding 도구를 사용하더라도 저장소 구조, 코딩 규칙, 보안 원칙, 검증 절차는 이 공통 지침을 기준으로 따른다. 특정 도구 전용 skill, command, MCP, plugin, connector가 있더라도 프로젝트 공통 규칙을 우회하지 않는다.
루트의 `AGENT.md`를 공통 기준 문서로 둔다. `AGENTS.md`, `CLAUDE.md` 또는 도구별 문서와 내용이 충돌하면 `AGENT.md`를 우선한다.
이 문서는 이 저장소에서 Codex를 포함한 AI 에이전트가 안전하고 일관되게 작업하기 위한 기본 지침이다.
저장소 루트와 모든 하위 경로에 적용한다. 하위 디렉터리에 더 구체적인 AI 지침 문서(`AGENT.md`, `AGENTS.md`, `CLAUDE.md` 등)가 생기면 해당 범위에서는 하위 문서가 우선한다.
